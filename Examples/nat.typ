...
type G[K,P] = (K -> P) -> (F_Nat[K] -> P);
type U[P] = /\K (K -> P);
type T[P] = /\K ((K -> U[P] -> K -> P) -> K -> P);
let 0' = (fun f x => x) : F_Nat[/\K K];
let S' = /\K fun n:K => (fun f x => f n):F_Nat[K];
let Z = /\P fun f:(/\K G[K,P]) =>
  (fun r => f (fun x => x):(/\K K -> P) 0'):U[P];
let Sc = /\P fun f:(/\K G[K,P]) =>
  (fun p r => f (fun s => p r (Z f) r) (S' p)) : T[P];
let fix = /\P fun f:(/\K G[K,P]) n:Nat =>
...
let rec eqN n:Nat m:Nat =
  n (\np. m (\mp. eqN np mp) False) (m (\mp. False) True)
;
