...
type Stream[A] = ?K F_Stream[A,K];

let cons = /\A (fun a:A s:Stream[A] => pair3 (pair a s) pi1 pi2);

let head = /\A (fun s:Stream[A] => s (\s1 a f.a s1));

let tail = /\A (fun s:Stream[A] => (s (\s1 a f.f s1)):Stream[A]);
...
let coiter = /\S/\A (fun f:(S -> Prod[S,A]) n:S =>
  (pair3 (pair n 
         ...
         (fun c => pair3 (pair (pi1 (f (pi1 c))) (pi2 c)) 
                    (fun c => pi2 (f (pi1 c))) (pi2 c)):T[A,S]):Stream[A]);
...
let corec = /\P/\A (fun f:G[A,P] n:P =>
  (pair3 (pair n (tail'' f)) (head'' f) (tail'' f)):Stream[A]);
