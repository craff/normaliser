...
type pp_token =
   ...
   | Pp_hbreak of int * int       (* complete break, more hor. in hov *)
...
and pp_output_function = ref(fun s -> output stdout (Bytes.unsafe_of_string s)) (* output function *)
and pp_flush_function = ref(fun () -> flush stdout) (* flushing function *)
;;

(* Output functions for the formatter *)
let pp_output_string s = !pp_output_function s 0 (String.length s)
      ...
      Scan_elem (left_tot,
       ...
       if left_tot < !pp_left_total then clear_scan_stack () else
        begin match tok with
           Pp_break (_, _) | Pp_hbreak (_, _) | Pp_tbreak (_, _) ->
            if ty then
             begin
...
(* To format a char *)
let print_char c =
  let s = String.make 1 c in print_as 1 s;;
...
let print_hbreak (width, offset) =
    if !pp_curr_depth < !pp_max_boxes then
      scan_push true
       {elem_size = (- !pp_right_total); token = Pp_hbreak (width,offset);
        length = width};;

let print_space () = print_break (1,0)
and print_cut () = print_break (0,0);;

let print_hspace () = print_hbreak (1,0)
and print_hcut () = print_hbreak (0,0);;
...
let set_margin n =
    if n >= 1 then
     begin
      pp_margin := n;
      pp_max_indent := min !pp_max_indent (!pp_margin - !pp_min_space_left);
      if !pp_max_indent < 0 then set_min_space_left (!pp_margin / 2);
      pp_rinit () end;;
...
let set_formatter_output_channel os =
  pp_output_function := (fun s -> output os (Bytes.unsafe_of_string s));
